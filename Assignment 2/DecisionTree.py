...
def print_tree(node, spacing=""):
    ...
    #     print(spacing + "Node header:", node.header)
    #     print(spacing + "Node rows:")
...
def getLeafNodes(node, leafNodes =[]):
    # Base case: we've reached a leaf
    if isinstance(node, Leaf) and node.rows is not None:
...
def randomPruning(List):
    for node in List:
        if(node.id ==0.0):
            List.remove(node)

    #num_to_select = int(len(List)*(random.randint(5,8))/10)
    num_to_select = int(len(List) * 4 / 10)
    rlist = random.sample(List,num_to_select)
    #Get Id's of all nodes
    randomList = []
    for list in rlist:
        randomList.append(list.id)
    return randomList
